...
function ListaPacienti({ token }) {
    ...
    const renderCell = (user, columnKey) => {
        ...
        let dataInt = new Date(user.dataInt).toDateString();
        let dataExt = verifyDate(user.dataExt);
        let flag = user.flag;

        function verifyDate(dataExt) {
            if (dataExt == null) {
                dataExt = today;
                return dataExt;
            }
    ...
    switch (columnKey) {
        ...
        case "dataInt":
            return (
                <Text color={flag == true ? "#B5B4B8" : "#050505"}>{dataInt}</Text>
            );
        case "dataExt":
            if (dataExt == today) {
                return (
                    <Text color={flag == true ? "#B5B4B8" : "#050505"}>Neexternat</Text>
                )
            }
            else {
                return (
                    <Text color={flag == true ? "#B5B4B8" : "#050505"}>{dataExt}</Text>
                );
            }
